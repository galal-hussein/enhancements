# KEP-5500: SemVer Operators and CEL Expressions for Node Affinity and Tolerations
...
Additionally, this enhancement enables complex version comparisons in **core/v1 NodeAffinity** using CEL expressions.
...
- Integrate Common Expression Language (CEL) into NodeAffinity to support complex version selection logic using `matchExpressionsWithCEL` field.
...
#### Story 2 — Complex versioning logic


As an operator, I want to handle scheduling of an application based on GPU version that might not be strict SemVer compatible, CEL can be used to normalize the version before adding the rule.
  ...
  affinity:
    nodeAffinity:
      requiredDuringSchedulingIgnoredDuringExecution:
        nodeSelectorTerms:
        - matchExpressionsCEL:
          - expression: semver_compare(metadata.labels['gpu-driver'] + ".0", '>=535.0.0')
```

#### Story 3 — Tolerate Running workloads on nodes with older versions of CNI
